...
function buildSummary(){
  ...
  for(const f of FUNDS){
    ...
    if(first[2] > 0 && last[2] > 0){
      ...
      let returns=[];
      for(let i=1;i<pos.length;i++) if(pos[i-1][2] > 0) returns.push(pos[i][2]/pos[i-1][2]-1);
      if(returns.length > 1){
        const mean=returns.reduce((a,b)=>a+b,0)/returns.length;
        const variance=returns.reduce((a,b)=>a+(b-mean)*(b-mean),0)/(returns.length-1);
        vol=Math.sqrt(variance)*Math.sqrt(252)*100;
      }
      let peak=-Infinity, minDD=0;
      for(const r of pos){peak=Math.max(peak,r[2]); minDD=Math.min(minDD, (r[2]/peak-1)*100)}
      mdd=minDD;
    }
...
function initControls(){
  el('metaLine').textContent = `${ROWS.length.toLocaleString('pt-PT')} observações · ${FUNDS.length} fundos · ${DATA.min_date} a ${DATA.max_date} · atualizado: ${new Date(DATA.updated_at).toLocaleString('pt-PT')}`;
...
function calculate(){const f=Number(el('calcFund').value),amount=Number(el('amount').value)||0,s=lookup(f,el('calcStart').value,'start'),e=lookup(f,el('calcEnd').value,'end');if(!s||!e){el('calcResult').innerHTML='<div class="card"><b>Sem dados válidos</b><span>—</span></div>';return}const units=amount/s[2],final=units*e[2],gain=final-amount,ret=(e[2]/s[2]-1)*100;el('calcResult').innerHTML='<div class="card"><b>Unidades</b><span>'+num(units,6)+'</span></div><div class="card"><b>Valor final</b><span>'+euro(final)+'</span></div><div class="card"><b>Ganho/perda</b><span class="'+(gain>=0?'pos':'neg')+'">'+euro(gain)+'</span></div><div class="card"><b>Retorno</b><span class="'+(ret>=0?'pos':'neg')+'">'+pct(ret)+'</span></div><div class="small" style="grid-column:1/-1">Cotação compra: '+num(s[2],6)+' em '+s[1]+' · cotação venda: '+num(e[2],6)+' em '+e[1]+' · Fundo: <span class="pill">'+FUNDS[f]+'</span></div>'}
...
loadData().then(()=>{initControls();renderSummary();update();calculate();}).catch(err=>{document.body.innerHTML='<main class="grid"><section class="panel error"><h1>Erro ao carregar dados</h1><p>'+err.message+'</p><p>Se estiveres a abrir localmente, usa um servidor local: <code>python -m http.server</code>.</p></section></main>';console.error(err);});
