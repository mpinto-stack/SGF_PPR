...
function filteredRows(){const q=(el('search')?.value||'').toLowerCase();return selectedRowsNoSearch().filter(r=>FUNDS[r[0]].toLowerCase().includes(q))}
...
function shiftDate(end, range){const d=new Date(end+'T00:00:00'); if(range==='1M') d.setMonth(d.getMonth()-1); if(range==='3M') d.setMonth(d.getMonth()-3); if(range==='1Y') d.setFullYear(d.getFullYear()-1); if(range==='3Y') d.setFullYear(d.getFullYear()-3); if(range==='5Y') d.setFullYear(d.getFullYear()-5); if(range==='YTD') return end.slice(0,4)+'-01-01'; return isoDate(d);}
...
function renderStatus(){const s=dataStatus(), box=el('updateStatus'); box.className='statusCard '+s.cls; box.innerHTML='<span class="statusDot"></span><div><b>'+s.title+'</b><small>'+s.text+'</small></div>'; const upd=DATA.updated_at?new Date(DATA.updated_at).toLocaleString('pt-PT'):'—'; el('metaLine').textContent=`${ROWS.length.toLocaleString('pt-PT')} observações · ${FUNDS.length} fundos · ${DATA.min_date} a ${DATA.max_date} · workflow: ${upd}`;}
...
function initControls(){
  ...
  ['fundSelect','startDate','endDate','metric','search','pageSize','page'].forEach(id=>{if(el(id)) el(id).addEventListener('input',update)});
  document.querySelectorAll('.quickRanges button').forEach(b=>b.addEventListener('click',()=>setRange(b.dataset.range)));
  document.querySelectorAll('.analysisTabs button').forEach(b=>b.addEventListener('click',()=>setTab(b.dataset.tab)));
  el('selectAllBtn').addEventListener('click',()=>{[...el('fundSelect').options].forEach(o=>o.selected=true);update();});
  el('rawTopBtn').addEventListener('click',()=>window.scrollTo({top:0,behavior:'smooth'}));
...
function setTab(tab){activeTab=tab; document.querySelectorAll('.analysisTabs button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab)); document.querySelectorAll('.tabPane').forEach(p=>p.classList.remove('active')); el('tab-'+tab).classList.add('active');}
function update(){renderCards();drawChart();renderAnalysis();renderDataTable()}

function statForRows(f, arr){
  arr=[...arr].sort((a,b)=>a[1].localeCompare(b[1])); const pos=arr.filter(r=>r[2]>0); if(!arr.length || pos.length<1) return null;
  const first=pos[0], last=pos[pos.length-1], ret=(last[2]/first[2]-1)*100; const days=Math.max(daysBetween(first[1],last[1]),1); const ann=(Math.pow(last[2]/first[2],365/days)-1)*100;
  let peak=-Infinity, mdd=0; for(const r of pos){peak=Math.max(peak,r[2]); mdd=Math.min(mdd,(r[2]/peak-1)*100)}
  let bestDay=null,worstDay=null,returns=[]; for(let i=1;i<pos.length;i++){const d=(pos[i][2]/pos[i-1][2]-1)*100; returns.push(d); if(!bestDay||d>bestDay.ret)bestDay={date:pos[i][1],ret:d}; if(!worstDay||d<worstDay.ret)worstDay={date:pos[i][1],ret:d};}
  let vol=null; if(returns.length>1){const rs=returns.map(x=>x/100); const mean=rs.reduce((a,b)=>a+b,0)/rs.length; const variance=rs.reduce((a,b)=>a+(b-mean)*(b-mean),0)/(rs.length-1); vol=Math.sqrt(variance)*Math.sqrt(252)*100;}
  return {f,nome:FUNDS[f],start:first[1],end:last[1],startQuote:first[2],endQuote:last[2],ret,ann,vol,mdd,bestDay,worstDay,n:arr.length,lastDate:last[1]};
}
function selectedOrAllStats(fundIdxs, sd, ed){const fs=new Set(fundIdxs); return [...byFund(ROWS.filter(r=>fs.has(r[0])&&r[1]>=sd&&r[1]<=ed)).entries()].map(([f,a])=>statForRows(f,a)).filter(Boolean);}
function horizonReturn(f, range){let end=DATA.max_date, start=range==='TOTAL'?DATA.min_date:shiftDate(end,range); if(start<DATA.min_date) start=DATA.min_date; const arr=ROWS.filter(r=>r[0]===f&&r[1]>=start&&r[1]<=end&&r[2]>0).sort((a,b)=>a[1].localeCompare(b[1])); if(arr.length<2) return null; return (arr[arr.length-1][2]/arr[0][2]-1)*100;}
function heatClass(v){ if(v===null||v===undefined||Number.isNaN(v)) return 'heatNeutral'; if(v>0) return 'heatGood'; if(v<0) return 'heatBad'; return 'heatNeutral';}
function renderAnalysis(){renderInsightCards();renderRanking();renderHorizons();renderAlerts();}
function renderInsightCards(){const stats=selectedOrAllStats(selectedFunds(),el('startDate').value,el('endDate').value).sort((a,b)=>b.ret-a.ret); const best=stats[0], worst=stats[stats.length-1]; const avg=stats.length?stats.reduce((a,b)=>a+b.ret,0)/stats.length:null; const maxDD=stats.length?stats.reduce((a,b)=>a.mdd<b.mdd?a:b):null; const cards=[['Melhor fundo',best?best.nome:'—',best?pct(best.ret):''],['Pior fundo',worst?worst.nome:'—',worst?pct(worst.ret):''],['Média dos selecionados',avg!==null?pct(avg):'—','retorno simples'],['Maior drawdown',maxDD?maxDD.nome:'—',maxDD?pct(maxDD.mdd):'']]; el('insightCards').innerHTML=cards.map(c=>'<div class="insightCard"><b>'+c[0]+'</b><strong>'+c[1]+'</strong>'+(c[2]?'<div class="small">'+c[2]+'</div>':'')+'</div>').join('');}
function renderRanking(){const stats=selectedOrAllStats(selectedFunds(),el('startDate').value,el('endDate').value).sort((a,b)=>b.ret-a.ret); const h=['#','Fundo','Início','Fim','Cot. inicial','Cot. final','Retorno','Anualizado','Vol. anual','Max drawdown','Melhor dia','Pior dia']; el('periodRankingTable').innerHTML='<thead><tr>'+h.map(x=>'<th>'+x+'</th>').join('')+'</tr></thead><tbody>'+stats.map((s,i)=>'<tr><td>'+(i+1)+'</td><td>'+s.nome+'</td><td>'+datePT(s.start)+'</td><td>'+datePT(s.end)+'</td><td>'+num(s.startQuote,4)+'</td><td>'+num(s.endQuote,4)+'</td><td class="'+(s.ret>=0?'pos':'neg')+'">'+pct(s.ret)+'</td><td class="'+(s.ann>=0?'pos':'neg')+'">'+pct(s.ann)+'</td><td>'+pct(s.vol)+'</td><td class="neg">'+pct(s.mdd)+'</td><td class="pos">'+(s.bestDay?datePT(s.bestDay.date)+' · '+pct(s.bestDay.ret):'—')+'</td><td class="neg">'+(s.worstDay?datePT(s.worstDay.date)+' · '+pct(s.worstDay.ret):'—')+'</td></tr>').join('')+'</tbody>';}
function renderHorizons(){const ranges=[['1M','1M'],['3M','3M'],['YTD','YTD'],['1A','1Y'],['3A','3Y'],['5A','5Y'],['Total','TOTAL']]; const funds=selectedFunds(); el('horizonReturnsTable').innerHTML='<thead><tr><th>Fundo</th>'+ranges.map(r=>'<th>'+r[0]+'</th>').join('')+'</tr></thead><tbody>'+funds.map(f=>'<tr><td>'+FUNDS[f]+'</td>'+ranges.map(r=>{const v=horizonReturn(f,r[1]); return '<td class="'+heatClass(v)+'">'+pct(v)+'</td>';}).join('')+'</tr>').join('')+'</tbody>';}
function renderAlerts(){const today=new Date().toISOString().slice(0,10); const funds=selectedFunds(); const stale=[], zeros=[], jumps=[]; for(const f of funds){const arr=ROWS.filter(r=>r[0]===f).sort((a,b)=>a[1].localeCompare(b[1])); const pos=arr.filter(r=>r[2]>0); if(pos.length){const age=daysBetween(pos[pos.length-1][1],today); if(age>7) stale.push(FUNDS[f]+' ('+age+' dias)');} if(arr.some(r=>r[2]===0)) zeros.push(FUNDS[f]); for(let i=1;i<pos.length;i++){const ret=Math.abs(pos[i][2]/pos[i-1][2]-1)*100; if(ret>10){jumps.push(FUNDS[f]+' · '+datePT(pos[i][1])+' · '+pct(ret)); if(jumps.length>8) break;}}}
  const card=(cls,title,items,okText)=>'<div class="alertCard '+cls+'"><b>'+title+'</b>'+(items.length?'<ul>'+items.slice(0,8).map(x=>'<li>'+x+'</li>').join('')+'</ul>':'<p class="small">'+okText+'</p>')+'</div>';
  el('alertsBox').innerHTML=card(stale.length?'bad':'good','Fundos sem cotação recente',stale,'Tudo parece atualizado nos fundos selecionados.')+card(zeros.length?'warn':'good','Cotações a zero',zeros,'Não encontrei cotações a zero nos fundos selecionados.')+card(jumps.length?'warn':'good','Variações diárias > 10%',jumps,'Não encontrei variações diárias anormais acima de 10%.');}

function renderCards(){const rows=selectedRowsNoSearch(),m=byFund(rows),all=[];m.forEach(a=>{if(a.length>=2){const b=a.find(z=>z[2]>0)||a[0];all.push({f:a[0][0],ret:b[2]>0?(a[a.length-1][2]/b[2]-1)*100:0})}});all.sort((a,b)=>b.ret-a.ret);const best=all[0],worst=all[all.length-1];const cards=[['Fundos selecionados',m.size],['Observações',rows.length],['Melhor no período',best?FUNDS[best.f]:'—',best?pct(best.ret):''],['Pior no período',worst?FUNDS[worst.f]:'—',worst?pct(worst.ret):''],['Intervalo',datePT(el('startDate').value)+' → '+datePT(el('endDate').value)]];el('cards').innerHTML=cards.map(c=>'<div class="card"><b>'+c[0]+'</b><span>'+c[1]+'</span>'+(c[2]?'<div class="small">'+c[2]+'</div>':'')+'</div>').join('')}
function valueLabel(v){const m=el('metric').value;if(m==='quote')return num(v,6);if(m==='base100')return num(v,2);return pct(v,2)}
function drawChart(){const canvas=el('chart'),ctx=canvas.getContext('2d'),rect=canvas.getBoundingClientRect(),DPR=window.devicePixelRatio||1;canvas.width=rect.width*DPR;canvas.height=rect.height*DPR;ctx.setTransform(DPR,0,0,DPR,0,0);const W=rect.width,H=rect.height,p={l:62,r:20,t:26,b:48};ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);LAST_POINTS=[];const rows=selectedRowsNoSearch(),m=byFund(rows),dates=[...new Set(rows.map(r=>r[1]))].sort();if(!dates.length){ctx.fillStyle='#64748b';ctx.font='15px system-ui';ctx.fillText('Sem dados para os filtros selecionados.',24,44);return;}let series=[],vals=[],ci=0;m.forEach((a,f)=>{const baseRow=a.find(z=>z[2]>0)||a[0],base=baseRow[2];let peak=-Infinity;const pts=a.map(r=>{let y=r[2];if(el('metric').value==='base100')y=base>0?r[2]/base*100:0;if(el('metric').value==='return')y=base>0?(r[2]/base-1)*100:0;if(el('metric').value==='drawdown'){peak=Math.max(peak,r[2]);y=peak>0?(r[2]/peak-1)*100:0}vals.push(y);return{d:r[1],raw:r[2],y:y}});series.push({f:f,pts:pts,color:COLORS[ci++%COLORS.length]})});let ymin=Math.min(...vals),ymax=Math.max(...vals);if(ymin===ymax){ymin-=1;ymax+=1}const yr=ymax-ymin;const x=d=>p.l+(dates.indexOf(d)/(Math.max(dates.length-1,1)))*(W-p.l-p.r);const y=v=>p.t+(ymax-v)/yr*(H-p.t-p.b);ctx.strokeStyle='#e5edf6';ctx.fillStyle='#64748b';ctx.font='12px system-ui';ctx.lineWidth=1;for(let i=0;i<=5;i++){const yy=p.t+i*(H-p.t-p.b)/5;ctx.beginPath();ctx.moveTo(p.l,yy);ctx.lineTo(W-p.r,yy);ctx.stroke();ctx.fillText(num(ymax-i*yr/5,el('metric').value==='quote'?3:1),8,yy+4)}for(let i=0;i<5;i++){const di=Math.round(i*(dates.length-1)/4);ctx.fillText(dates[di],p.l+i*(W-p.l-p.r)/4-30,H-17)}series.forEach(s=>{ctx.strokeStyle=s.color;ctx.lineWidth=2.4;ctx.beginPath();s.pts.forEach((pt,i)=>{const xx=x(pt.d),yy=y(pt.y);LAST_POINTS.push({x:xx,y:yy,date:pt.d,f:s.f,value:pt.y,raw:pt.raw,color:s.color});if(i)ctx.lineTo(xx,yy);else ctx.moveTo(xx,yy)});ctx.stroke()});let lx=p.l,ly=18;series.slice(0,8).forEach(s=>{ctx.fillStyle=s.color;ctx.fillRect(lx,ly-9,10,10);ctx.fillStyle='#334155';ctx.fillText(FUNDS[s.f].slice(0,23),lx+14,ly);lx+=175;if(lx>W-180){lx=p.l;ly+=18}})}
